...
void *malloc_from_heap(int32_t size)
{
    ...
    if (SUBREGIONS_512(size) < SUBREGIONS_1024(size) || !SUBREGIONS_1024(size))
    {
        retVal = getAllocation((SUBREGIONS_512(size)), (BLOCK_4K1_START), (BLOCK_4K1_END - 1), REGION_4K1_BASE_ADDR, 0, BLOCK_SIZE_1);
        RETURN_VALID;

        retVal = getAllocation((SUBREGIONS_512(size)), (BLOCK_4K2_START + 1), BLOCK_4K2_END, REGION_4K2_BASE_ADDR, -15, BLOCK_SIZE_1);
        RETURN_VALID;

        retVal = getAllocation((SUBREGIONS_512(size)), (BLOCK_4K3_START), (BLOCK_4K3_END - 1), REGION_4K3_BASE_ADDR, -22, BLOCK_SIZE_1);
        RETURN_VALID;
    }
    ...
    {
        retVal = getAllocation((SUBREGIONS_1024(size)), (BLOCK_8K1_START + 1), (BLOCK_8K1_END - 1), REGION_8K1_BASE_ADDR, -7, BLOCK_SIZE_2);
        RETURN_VALID;

        retVal = getAllocation((SUBREGIONS_1024(size)), (BLOCK_8K2_START + 1), (BLOCK_8K2_END), REGION_8K2_BASE_ADDR, -31, BLOCK_SIZE_2);
        RETURN_VALID;
    }
...
void free(void *baseAdd)
{
    ...
    {
        if (heapMetadata_g[index].address == baseAdd)
        {
